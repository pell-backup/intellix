...
DATA_ORACLE_PAY_IN_NATIVE_CONSUMER_ADDRESS=$(ssh hardhat "cat $HARDHAT_DVS_PATH/ScriptOraclePayInNativeConsumer.json" | jq -r .address)

## create a new task
cast send "$DATA_ORACLE_PAY_IN_NATIVE_CONSUMER_ADDRESS" "requestScriptTask(uint256,bytes)" 0 "0x10101001" --private-key "$ADMIN_KEY" --rpc-url "$ETH_RPC_URL"
...
RESULT=$(cast call "$DATA_ORACLE_PAY_IN_NATIVE_CONSUMER_ADDRESS" "price()" --private-key "$ADMIN_KEY" --rpc-url "$ETH_RPC_URL" | cast to-dec)
assert_gt "$RESULT" "0"

# cast call "$DATA_ORACLE_PAY_IN_NATIVE_CONSUMER_ADDRESS" "allTaskResponses(uint32)" $((TASK_NUMBER - 1))
