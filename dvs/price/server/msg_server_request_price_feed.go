...
func (d Server) broadcastVoteRequestPriceFeed(ctx sdktypes.Context, task *types.RequestPriceFeedIn, priceFeed *types.PriceFeedParam, rawPrices map[string]math.LegacyDec) error {
	...
	msg := pricetypes.MsgVoteRequestPriceFeed{
		...
		OperatorId:  d.GetOperatorAddress(ctx),
	...
	bls := pkgutils.SignWithBLS(d.blsKeyPair, d.getMsgBytes(&msg))
...
func (d Server) startCollectEnoughPriceFeedTxs(ctx sdktypes.Context, requestId []byte) ([]*pricetypes.MsgVoteRequestPriceFeed, error) {
	...
	// get block height
	block, err := d.GetLatestBlock(ctx)
	if err != nil {
...
func (d Server) verifyOperatorEvents(ctx sdktypes.Context, datas []tx_listener.EventData[*pricetypes.MsgVoteRequestPriceFeed]) ([]*pricetypes.MsgVoteRequestPriceFeed, bool) {
	...
	for _, v := range datas {
		if operator, ok := operatorMaps[v.Data.OperatorId]; ok {
			...
			err := pkgutils.VerifyBLSSignature(operator.Pubkeys, d.getMsgBytes(v.Data), v.Data.BlsSignature)
			if err != nil {
				continue
			}
...
func (d Server) collectEvents(ctx context.Context, operatorMaps map[string]*avsitypes.Operator, ch *tx_listener.EventChannel[*pricetypes.MsgVoteRequestPriceFeed], eventData *[]*pricetypes.MsgVoteRequestPriceFeed, waitChan chan struct{}) {
	...
	case data, ok := <-ch.Data:
		...
		if ok && pkgutils.VerifyBLSSignature(operator.Pubkeys, d.getMsgBytes(data.Data), data.Data.BlsSignature) == nil {
			mu.Lock()
