...
func (server RequestServer) RequestPriceFeed(ctx context.Context, request *types.RequestPriceFeedIn) (*types.RequestPriceFeedOut, error) {
	...
	rawPrices, err := fetchRawPricesEnhanced(pkgContext, server.Logger(), request.PriceFeed.BaseSymbol, request.PriceFeed.QuoteSymbol, ToPriceTickConverterByDataSource(server.tickConverterConfig), server.cmcApiKey)
	if err != nil {
